...
Round start beat makes it so if you run ConductedTimer.start() at "1.1" beats, it'll round to 1
...
Round start beat makes it so if you run ConductedAnimationPlayer.start() at "1.1" beats, it'll round to 1

you can also set the start beat in code, to make it easier to sync certain things.

If you use call functions, it will repeatedly call the same function unless you put code to disable repeated calling
